...
impl fmt::Display for DomainError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingAnamnesisSeed => {
                f.write_str("cycle requires a seed repetition unless allow_empty_cycle is true")
            }
            Self::ExistingAnamnesisCycleRequired => {
                f.write_str("append-rep requires an existing cycle")
            }
